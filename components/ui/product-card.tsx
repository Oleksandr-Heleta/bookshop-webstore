  ...
  return (
    <div  className="bg-white group  rounded-xl border border-amber-800 p-3 space-y-4" >
      ...
      {data.isNew && <div className=" bg-red-600 text-white rounded-lg p-1 tracking-wider text-xs font-semibold">
        ...
        {data.isSale && <div className=" bg-orange-500 text-white text-center rounded-lg p-1 text-xs font-semibold"> 
          - {data.sale} %
        </div>}
      </div>
        <Image
        ...
        <div className="opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition absolute w-full px-6 bottom-5">
          <div className="flex gap-x-6 justify-center">
            ...
            <IconButton
              onClick={onAddToCart}
              icon={<ShoppingCart size={20} className="text-white bg-inherit" />}
            />
          </div>
        </div>
      </div>
      <div>
        <p onClick={handleClick} className="cursor-pointer font-semibold text-lg text-amber-950 hover:scale-105 origin-left transition">{data.name}</p>
       
      </div>
      <div className="flex item-center justify-between">
        <CheckSale isSale={data.isSale} percent={data.sale} price={data.price}/>
